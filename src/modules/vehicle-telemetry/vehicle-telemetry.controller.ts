...
@Controller('v1/telemetry/vehicle')
export class VehicleTelemetryController {
    ...
    async ingestVehicleTelemetry(@Body() dto: CreateVehicleTelemetryDto) {
        try {
            ...
            return setCreateSuccess({ message: 'Telemetry data ingested successfully', data: telemetry });
        } catch (error) {
    ...
    async getLatestTelemetry(@Param('vehicleId') vehicleId: string) {
        try {
            ...
            return setSuccess({ message: 'Latest telemetry retrieved successfully', data: telemetry });
        } catch (error) {
